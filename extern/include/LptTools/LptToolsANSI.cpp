...
int			iPsOsVersion;
	...
	iPsOsVersion=GetSystemDirectoryA(cPath,sizeof(cPath)-24);
	strcpy(cPath+iPsOsVersion,"\\Drivers\\" DRIVER_NAME ".sys");



	hSCService=CreateServiceA(hSCManager ,						// handle to service control manager
...
WIN32_FIND_DATAA    sFind;
int					iLen;

	bUseService=FALSE;

															// Prüfe ob der Treiber im Windows-System-Verzeichnis ist
	iLen=GetSystemDirectoryA(cBuffer1,sizeof(cBuffer1));
	strncpy(cBuffer1+iLen,"\\Drivers\\" DRIVER_NAME ".sys",sizeof(cBuffer1)-iLen);
	if(!bForceCopy)hFind=FindFirstFileA(cBuffer1,&sFind);

	if(hFind==INVALID_HANDLE_VALUE) 							// Treiber neu kopieren
		{
		iLen=GetModuleFileNameA(GetModuleHandle(0),cBuffer2,sizeof(cBuffer2));
		for(;iLen>0;iLen--)
			{
		...
		CopyFileA(cBuffer2,cBuffer1,FALSE);
		}
	else{
		FindClose(hFind);
		}
	...
	if(!hSCManager)return FALSE;
		...
		hSCService = OpenServiceA(hSCManager,DRIVER_NAME, SERVICE_QUERY_STATUS);
	if(!hSCService)
		{
		LptServiceCreate();
		hSCService = OpenServiceA(hSCManager,DRIVER_NAME, SERVICE_QUERY_STATUS);
	...
	CloseServiceHandle(hSCService);
	hSCService = OpenServiceA(hSCManager,DRIVER_NAME, SERVICE_START);
...
HKEY	hKey;
	...
	RegOpenKeyExA(HKEY_LOCAL_MACHINE,"SYSTEM\\CurrentControlSet\\Services\\" DRIVER_NAME,0,KEY_ALL_ACCESS,&hKey);
	RegDeleteValueA(hKey,"DeleteFlag");
	RegCloseKey(hKey);
...
BOOL		bOk=TRUE;
	...
	hSCService = OpenServiceA(hSCManager,DRIVER_NAME,DELETE);
...
int				i;
	...
	RegOpenKeyExA(HKEY_DYN_DATA,"Config Manager\\Enum",0,KEY_ENUMERATE_SUB_KEYS|KEY_QUERY_VALUE,&hKey);

	
	dwDummyLength = MAX_PATH;
	dwKeyCount    = 0;
	
	while(RegEnumKeyExA(hKey,dwKeyCount++,cKeyName,&dwDummyLength,NULL,NULL,NULL,&sDummyFileTime)!=ERROR_NO_MORE_ITEMS)
		{
		dwDummyLength = MAX_PATH;
		}
	...
	for(dwKeyIndex=0; dwKeyIndex<dwKeyCount; dwKeyIndex++)
		...
		if(RegOpenKeyExA(HKEY_DYN_DATA, cKeyName, 0, KEY_ENUMERATE_SUB_KEYS|KEY_QUERY_VALUE, &hKey) != ERROR_SUCCESS)
			{
			continue;
			}
		...
		RegQueryValueExA(hKey, "Problem", NULL, &dwDataType, NULL, &dwDataSize);
		...
		if(dwDataType == REG_BINARY)
			...
			if(RegQueryValueExA(hKey,"Problem",NULL,NULL,pData,&dwDataSize)==ERROR_SUCCESS)
				{
			...
			RegQueryValueExA(hKey,"HardwareKey",NULL,&dwDataType,(unsigned char*)cHardwareSubKey,&dwDataSize);
			...
			if(!bHasProblem && strlen(cHardwareSubKey) > 0)		// Do we have no problem, and a non-null Hardware sub-key?
				...
				if(RegOpenKeyExA(HKEY_LOCAL_MACHINE, cKeyName, 0, KEY_ENUMERATE_SUB_KEYS|KEY_QUERY_VALUE, &hKey) != ERROR_SUCCESS)
					{
					continue;
					}
				...
				dwDataSize = MAX_PATH;
				RegQueryValueExA(hKey,"PortName",NULL,&dwDataType,(unsigned char*)cPortName,&dwDataSize);
				...
				if(strstr(cPortName,"LPT")!=NULL)				// Make sure it has LPT in it
					...
					strcat(cKeyName, "\\");
					strcat(cKeyName, pKeyList[dwKeyIndex]);
					RegOpenKeyExA(HKEY_DYN_DATA, cKeyName, 0, KEY_ENUMERATE_SUB_KEYS|KEY_QUERY_VALUE, &hKey);

					dwDataSize = sizeof(wAllocation);
					RegQueryValueExA(hKey,"Allocation",NULL,&dwDataType,(unsigned char*)wAllocation,&dwDataSize);
...
int			i;
	...
	iFound = 0;
																...
																// Open the registry
	if(RegOpenKeyExA(HKEY_LOCAL_MACHINE,"SYSTEM\\CurrentControlSet\\Services\\Parport\\Enum",0,KEY_ENUMERATE_SUB_KEYS|KEY_QUERY_VALUE,&hKey) == ERROR_SUCCESS)
		{
		dwDummyLength = MAX_PATH;
		dwValueCount  = 0;

		while(RegEnumValueA(hKey,dwValueCount,cKeyName,&dwDummyLength,NULL,&dwValueType,NULL,NULL)!=ERROR_NO_MORE_ITEMS)
			{
			dwDummyLength = MAX_PATH;

			RegQueryValueExA(hKey, cKeyName, NULL, &dwValueType, NULL, &dwDataSize);
			...
			if(dwValueType==REG_SZ && cKeyName[0]>='0' && cKeyName[0]<='9')
				...
				pData           = new BYTE[dwDataSize   ];
				RegQueryValueExA(hKey,cKeyName,NULL,&dwValueType,pData,&dwDataSize);
				...
				if(dwValueType == REG_DWORD && strcmp(cKeyName, "Count") == 0 && dwDataSize == 4)
					{
					RegQueryValueExA(hKey,cKeyName,NULL,&dwValueType,(BYTE*)&dwVal,&dwDataSize);
					iLptCount = dwVal;
					}
				}
...
//*****************************************************************************
																// Open the registry
	if(RegOpenKeyExA(HKEY_LOCAL_MACHINE,"SYSTEM\\CurrentControlSet\\Enum",0,KEY_ENUMERATE_SUB_KEYS|KEY_QUERY_VALUE,&hKey) == ERROR_SUCCESS)
		{
		...
		while(RegEnumKeyA(hKey,dwCount0,cTemp+iPos0,sizeof(cTemp)-iPos0)!=ERROR_NO_MORE_ITEMS)
			{
			dwCount0++;

			if(RegOpenKeyExA(HKEY_LOCAL_MACHINE,cTemp,0,KEY_ENUMERATE_SUB_KEYS|KEY_QUERY_VALUE,&hKey0) != ERROR_SUCCESS)
				{
				continue;
				}
			...
			while(RegEnumKeyA(hKey0,dwCount1,cTemp+iPos1,sizeof(cTemp)-iPos1)!=ERROR_NO_MORE_ITEMS)
				{
				dwCount1++;

				if(RegOpenKeyExA(HKEY_LOCAL_MACHINE,cTemp,0,KEY_ENUMERATE_SUB_KEYS|KEY_QUERY_VALUE,&hKey1) != ERROR_SUCCESS)
					{
					continue;
					}
				...
				while(RegEnumKeyA(hKey1,dwCount2,cTemp+iPos2,sizeof(cTemp)-iPos2)!=ERROR_NO_MORE_ITEMS)
					{
					dwCount2++;	

					if(RegOpenKeyExA(HKEY_LOCAL_MACHINE,cTemp,0,KEY_ENUMERATE_SUB_KEYS|KEY_QUERY_VALUE,&hKey2) != ERROR_SUCCESS)
						{
						continue;
						}
					...
					RegQueryValueExA(hKey2,"Class",NULL,&dwValueType,(BYTE*)cValue,&dwDataSize);
					...
					if(RegOpenKeyExA(hKey2,"Device Parameters",0,KEY_ENUMERATE_SUB_KEYS|KEY_QUERY_VALUE,&hKey3) != ERROR_SUCCESS)
						{
						RegCloseKey(hKey2);
						continue;
						}
					...
					RegQueryValueExA(hKey3,"PortName",NULL,&dwValueType,(BYTE*)cValue,&dwDataSize);
					RegCloseKey(hKey3);
					RegCloseKey(hKey2);
...
//*****************************************************************************
	...
	for(uIndex=0;uIndex<10;uIndex++)
		...
		if(RegOpenKeyExA(HKEY_LOCAL_MACHINE,pStr,0,KEY_ENUMERATE_SUB_KEYS|KEY_QUERY_VALUE,&hKey)!=ERROR_SUCCESS)
			{
			delete pStr;
			continue;
			}
		...
		while(RegEnumValueA(hKey,dwValueCount,cKeyName,&dwDummyLength,NULL,&dwValueType,NULL,NULL)!=ERROR_NO_MORE_ITEMS)
			...
			*/
			...
			if(dwValueType==REG_RESOURCE_LIST && !strcmp(cKeyName,"AllocConfig"))
				...
				if(RegQueryValueExA(hKey,cKeyName,NULL,&dwValueType,(BYTE*)pData,&dwDataSize)!=ERROR_SUCCESS)
					{
					delete pData;
					continue;
					}
