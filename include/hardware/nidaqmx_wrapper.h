/*
    ...
    Contact: christian.breitwieser@tugraz.at
*/
...
typedef int32 (CALLBACK* DAQmxCreateTaskType) (const char[], TaskHandle*);
typedef int32 (CALLBACK* DAQmxGetErrorStringType) (int32, char[], uInt32);
typedef int32 (CALLBACK* DAQmxGetExtendedErrorInfoType) (char[], uInt32);
...
class NIDaqmxWrapper
{
  ...
  {
    dll_handle_ = LoadLibrary("NIDAQmx.lib");
    if (dll_handle_ == NULL)
  ...
  {
    return(ni_DAQmxStartTask_ptr_(handle) );
  }

  int32 stopTask(TaskHandle handle)
  {
    return(ni_DAQmxStopTask_ptr_(handle) );
  }

  int32 clearTask(TaskHandle handle)
  {
    return(ni_DAQmxClearTask_ptr_(handle) );
  }

  int32 createTask(const char task_name[], TaskHandle* handle)
  {
    return(ni_DAQmxCreateTask_ptr_(task_name, handle) );
  }

  int32 getErrorString(int32 error_code, char error_string[], uInt32 buffer_size)
  {
    return(ni_DAQmxGetErrorString_ptr_(error_code, error_string, buffer_size) );
  }

  int32 getExtendedErrorInfo(char error_string[], uInt32 buffer_size)
  {
    return( ni_DAQmxGetExtendedErrorInfo_ptr_(error_string, buffer_size) );
  }

  int32 readAnalogF64(TaskHandle handle, int32 num_samps_per_chan, float64 timeout,
                      bool32 fill_mode, float64 read_array[], uInt32 array_size_in_samps,
                      int32* samps_per_chan_read, bool32* reserverd)
  {
    return( ni_DAQmxReadAnalogF64_ptr_(handle, num_samps_per_chan, timeout,
                    fill_mode, read_array, array_size_in_samps, samps_per_chan_read, reserverd) );
  }

  int32 createAIVoltageChan(TaskHandle handle, const char physical_channel[],
                            const char name_to_assign_to_ch[], int32 terminal_config,
                            float64 min_val, float64 max_val, int32 units,
                            const char custom_scale_name[])
  {
    return( ni_DAQmxCreateAIVoltageChan_ptr_(handle, physical_channel, name_to_assign_to_ch,
  ...
  {
    return( ni_DAQmxCfgSampClkTiming_ptr_(handle, source, rate, active_edge,
